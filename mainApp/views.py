...
from django.db.models import Q # Multiple quiry par kaam karna hai to ye import karenge
from random import randrange
from django.conf import settings
from django.core.mail  import send_mail
from boot6.settings import RAZORPAY_API_KEY,RAZORPAY_API_SECRET_KEY
import razorpay
from .models import *
...
def shop(Request,mc,sc,br):    
    if(mc=='All' and sc=='All' and br=='All'):
      ...
      data=Product.objects.filter(subcategory=Subcategory.objects.get(name=sc),brand=Brand.objects.get(name=br))   
    
    elif(mc!='All' and sc!='All' and br!='All'):
      data=Product.objects.filter(maincategory=Maincategory.objects.get(name=mc),subcategory=Subcategory.objects.get(name=sc),brand=Brand.objects.get(name=br))
...
def loginPage(Request):
  if(Request.method=="POST"):
    ...
    if(user is not None):
      ...
      messages.error(Request,"Invalid Valid user or Password, Please try again ! ! ! ")
  ...
  if(Request.method=="POST"):
    ...
    if(p==cp):
      ...
      if(user):
        ...
        message = "Hellow "+b.name+"\nThanks to create a Buyer Account with us\nNow can Buy Our Latest Product : \nTeam E shop"
...
def addToCart(Request,id):
 # Request.session.flush()
  cart= Request.session.get("cart",None) #cart hai provide hoga verna None milega
  p = Product.objects.get(id=id) #product ko get kiya
  if(cart is None):       #agar cart mai kuch item nhii hai.
     cart= {str(p.id):{'pid':p.id,'pic':p.pic1.url,'name':p.name,'color':p.color,'size':p.size,'price':p.finalprice,'qty':1,'total':p.finalprice,'maincategory':p.maincategory.name,'subcategory':p.subcategory.name,'brand':p.brand.name,}}#key and value
  else:
      if(str(p.id) in cart):   # agar cart mai item hai to
         item = cart[str(p.id)]
         item['qty']=item['qty']+1
         ...
         cart[str(p.id)]=item
           ...
           cart.setdefault(str(p.id),{'pid':p.id,'pic':p.pic1.url,'name':p.name,'color':p.color,'size':p.size,'price':p.finalprice,'qty':1,'total':p.finalprice,'maincategory':p.maincategory.name,'subcategory':p.subcategory.name,'brand':p.brand.name})# dictionary mai jab bhi item add karege to set default hota hain 
...
def deleteCart(Request,pid):
       cart=Request.session.get('cart',None)
       if(cart): #agar cart hai
          for key in cart.keys(): #loop se key ko founded karenge 
             if(str(pid)==key): 
               del cart[key]
               break
          Request.session['cart']=cart    
...
def updateCart(Request,pid,op):
       cart=Request.session.get('cart',None)
       if(cart): #agar cart hai
          for key,value in cart.items(): # key se found karna hai value se update karna hain 
            if(str(pid)==key):  
               if(op=='inc'): 
                 ...
                 value['qty']=value['qty']-1
                 value['total']=value['total']-value['price'] 
               cart[key]=value # value jo bhi update hoi osse cart ki key mai assing kar de taki value update ho jaye
               break
          Request.session['cart']=cart  # session mai cart ko update kiya hai ye line main  
       return redirect("/cart")
...
def orderPage(Request):
  if(Request.method=='POST'):
    ...
    user = Buyer.objects.get(username=Request.user.username) # user foundout ki nitin hai ya koe or
    cart = Request.session.get('cart',None)
    if(cart is None):
          redirect("/cart/")
    else:
          ...
          if(mode=="COD"):
              ...
              orderCurrency = "INR"
              paymentOrder = client.order.create(dict(amount=orderAmount,
              currency=orderCurrency,payment_capture=1))
...
@login_required(login_url='/login/')
def paymentSuccess(request,rppid,rpoid,rpsid): #ry payment id, order id , rasident signature id.
    buyer = Buyer.objects.get(username=request.user)
...
def contactPage(Request):
  if(Request.method=="POST"): 
    c = Contact()   # contact create kiya hain
    ...
    c.save()
    messages.success(Request,"Thank to shere your Quiry with us !! Our team will contact you soon ")
  return render(Request,"contact.html") 


def searchPage(Request):
  search = Request.POST.get('search')
  data = Product.objects.filter(Q(name__icontains=search)|Q(color__icontains=search)|Q(size__icontains=search)|Q(stoke__icontains=search)|Q(description__icontains=search))
  maincategory=Maincategory.objects.all()
  subcategory=Subcategory.objects.all()
  brand=Brand.objects.all()
...
def forgetOTP(Request):
   if(Request.method=="POST"):     # pehle ye check hoga 
       otp= Request.POST.get('otp')
       username = Request.session.get('resetuser',None)
       if(username):               #phir ye check hoga
          buyer = Buyer.objects.get(username=username)
          if(int(otp)==buyer.otp): # phir ye check hoga
           return redirect("/forget-password/")  
          else:
            messages.error(Request,"Invalid OTP ! ! ! ! !")
        
       else:
          messages.error(Request,"UnAutherized Username")
